		...
		/*具体的各栏行记录 先用空的二位数组占位*/
		 ...
		 putDates(new BorrowDetail());//获取数据库数据放置table中		 
		  ...
		  jscrollpane.setBounds(20, 22, 607, 188);
			...
			addFeedBackbutton = new JButton("问题反馈");
			addFeedBackbutton.setFont(new Font("Dialog", Font.BOLD, 15));
			addFeedBackbutton.setBounds(80, 11, 103, 31);
			jf.getContentPane().add(addFeedBackbutton);
			...
			getFeedBackbutton = new JButton("我的反馈");
			getFeedBackbutton.setFont(new Font("Dialog", Font.BOLD, 15));
			getFeedBackbutton.setBounds(480, 11, 103, 31);
			jf.getContentPane().add(getFeedBackbutton);

			getFeedBackbutton.addActionListener(new ActionListener() {
				public void actionPerformed(ActionEvent e) {
					new MyFeedBack();
				}
			});
