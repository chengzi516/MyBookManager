...
public class UserDao {
    ...
    public List<Map<String, Object>> findUserPortrait(Connection connection, int id) throws SQLException {
        PreparedStatement ps = connection.prepareStatement("SELECT d.type_name typeName,COUNT(1) num FROM borrowdetail a JOIN `user` b ON a.user_id = b.id JOIN book c ON a.book_id = c.id JOIN book_type d ON c.type_id = d.id WHERE a.user_id = ? GROUP BY d.type_name");
        ps.setInt(1, id);
